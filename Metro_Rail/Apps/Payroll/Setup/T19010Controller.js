app.controller("T19010Controller", ["$scope", "Service", "Data", "$window", "$filter",
    function ($scope, Service, Data, $window, $filter) { //$location,
        ...
        loadItemData();
        loadDesignationData();
        $scope.obj.T_SAL_INC_DATE = new Date();
        $scope.obj.T_PRO_JOING_DATE = new Date();
        $scope.obj.T_JOINING_DATE = new Date();


        function loadItemData() {
            loader(true)
            var load = Service.loadDataWithoutParm('/T19010/GetGradeItem');
        ...
        function loadDesignationData() {
            loader(true)
            var load = Service.loadDataWithoutParm('/T19010/GetDesignationItem');
            load.then(function (returnData) {
                $scope.obj.DesignationList = JSON.parse(returnData);
                loader(false)
            });
        }
        ...
        $scope.updateDeductionTotal = function () {
            let total = 0;
        ...
        $scope.Save_Click = function () {
            ...
            $scope.obj.T19010.T_DESIGNATION_CODE = $scope.obj.ddlItemDesignation.T_DESIGNATION_CODE;
            $scope.obj.T19010.T_GRADE_CODE = $scope.obj.ddlItem.T_GRADE_CODE;
            ...
            $scope.obj.T19010.T_SAL_INC_DATE = $filter('date')($scope.obj.T_SAL_INC_DATE, 'dd-MM-yyyy');
            $scope.obj.T19010.T_SAL_INC_DATE = $filter('date')($scope.obj.T_SAL_INC_DATE, 'dd-MM-yyyy');
            $scope.obj.T19010.T_PRO_JOING_DATE = $filter('date')($scope.obj.T_PRO_JOING_DATE, 'dd-MM-yyyy');
            $scope.obj.T19010.T_JOINING_DATE = $filter('date')($scope.obj.T_JOINING_DATE, 'dd-MM-yyyy');
        ...
        $scope.Print_Click = function () {            
            $window.open("../T19010/T19010_GetEmpInfoWithSalary?empCode=" + $scope.obj.T19010.T_EMP_CODE, "_blank");
        }
        function clear() {
            $scope.obj.T19010 = {};
            loadPaymentGridData();
        ...
        $scope.onGradeChange = function () {
            $scope.obj.T19010.T_PAY_SCALE = $scope.obj.ddlItem.T_PAY_SCALE;
        ...
        $scope.setClientRow = function (ind, data) {
            ...
            //$scope.obj.ddlItemDesignation.T_DESIGNATION_CODE = data.T_DESIGNATION_CODE
            //$scope.obj.ddlItem.T_DESIGNATION_CODE = data.T_GRADE_CODE
