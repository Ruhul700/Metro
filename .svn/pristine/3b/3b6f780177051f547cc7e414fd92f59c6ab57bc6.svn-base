...
{
    public class T13003DAL:CommonDAL
    {
        public DataTable GetVoucherList(string center, string fromDate, string toDate)
        {
            DataTable sql = new DataTable();
            var condition = ""; //(VERIFY_FLG IS NULL or VERIFY_FLG = '') AND
            if (center == "0")
            { condition = $" (POSTING_FLG IS NULL OR POSTING_FLG='') AND VERIFY_FLG ='1' AND CONVERT(date, VOUCHER_DATE ,104) BETWEEN CONVERT(date, '{fromDate}' ,104) AND CONVERT(date, '{toDate}' ,104)"; }
            else { condition = $"(POSTING_FLG IS NULL OR POSTING_FLG='') AND VERIFY_FLG ='1' AND T12005.CENTER_CODE='{center}' AND CONVERT(date, VOUCHER_DATE ,104) BETWEEN CONVERT(date, '{fromDate}' ,104) AND CONVERT(date, '{toDate}' ,104)"; }
            ...
            sql = Query($"SELECT ROW_NUMBER() OVER (ORDER BY  CAST(VOUCHER_NO as int) DESC) SL_NO, VOUCHER_NO, VOUCHER_DATE, T12005.CENTER_NAME, T12005.CENTER_CODE, T12006.VAUCHER_TYPE_NAME, T12006.VAUCHER_TYPE_CODE, T12007.TRANSACTION_TYPE_NAME, T12007.TRANSACTION_TYPE_CODE,T12007.TRANSACTION_TYPE_CODE, T12008.PARTY_TYPE_NAME,T12008.PARTY_TYPE_CODE, T12009.PARTY_NAME , T12009.PARTY_CODE,TOTAL_DEBIT,TOTAL_CREDIT, CASE  WHEN POSTING_FLG IS NULL OR POSTING_FLG='' THEN '0' ELSE '1' END AS POSTING_FLG ,AT13001.ENTRY_USER, T11020.T_USER_NAME FROM AT13001 JOIN T12005 ON AT13001.CENTER_CODE =T12005.CENTER_CODE JOIN T12006 ON AT13001.VAUCHER_TYPE_CODE =T12006.VAUCHER_TYPE_CODE JOIN T12007 ON AT13001.TRANSACTION_TYPE_CODE =T12007.TRANSACTION_TYPE_CODE JOIN T12008 ON AT13001.PARTY_TYPE_CODE =T12008.PARTY_TYPE_CODE JOIN T12009 ON AT13001.PARTY_CODE =T12009.PARTY_CODE JOIN T11020 ON AT13001.ENTRY_USER =T11020.T_USER_ID WHERE {condition} ORDER BY CAST(VOUCHER_NO as int) DESC");
            return sql;
        }

        public string PostingData(List<T13001Data> dataList, string user)
        {
            var sms = "";
            ...
            foreach (var i in dataList)
            {
                try
                {
                    Command($"UPDATE AT13001 SET POSTING_FLG='{i.POSTING_FLG}',POSTING_DATE='{date}',POSTING_USER='{user}' WHERE VOUCHER_NO='{i.VOUCHER_NO}'");
